  ...
  A.Spine n l -> B.Pat $ foldl (B.:+:) np $ s2n stk <$> l
    where np = B.Var $ case getIndex stk n of
            Nothing -> case getExists stk n of
              ...
              Nothing -> B.Con n
            Just i  -> B.DeBr i
  A.Abs n ty s -> B.Abs (s2n stk ty) $ s2n (putName stk n) s
...
cons2form stk c = case c of
  ...
  A.Bind A.Forall n ty c -> B.Bind (s2n stk ty) 
...
n2s stk@(UnStack i lst) n = case n of
  B.Abs ty n -> A.Abs next (n2s stk ty) $ n2s (UnStack (i+1) $ next:lst) n
    where next = '@':show i
